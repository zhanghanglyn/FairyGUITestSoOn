...
public class PoolManager
{
    ...
    public ObjPool<T> CreateObjPool<T>( int _initNum , System.Func<T> _create_func, System.Action<T, bool> _setActive_func, System.Action<T> _clear_func = null , int _upNum = 2)
    {
        ...
        {
            ObjPool<T> temp_pool = new ObjPool<T>();
            temp_pool.InitObjPool(_initNum, _create_func, _setActive_func, _clear_func, _upNum);
            m_ObjPoolList.Add(temp_pool);
            m_TypeList.Add(typeof(T), m_ObjPoolList.Count - 1);
        }

        ObjPool<T> poolObj = m_ObjPoolList[m_TypeList[typeof(T)]] as ObjPool<T>;

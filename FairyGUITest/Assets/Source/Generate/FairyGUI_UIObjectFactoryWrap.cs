...
public class FairyGUI_UIObjectFactoryWrap
{
	...
	static int SetPackageItemExtension(IntPtr L)
	{
		try
		{
			...
			if (count == 2 && TypeChecker.CheckTypes<FairyGUI.UIObjectFactory.GComponentCreator>(L, 2))
			{
	...
	static int SetLoaderExtension(IntPtr L)
	{
		try
		{
			...
			if (count == 1 && TypeChecker.CheckTypes<FairyGUI.UIObjectFactory.GLoaderCreator>(L, 1))
			{
	...
	static int NewObject(IntPtr L)
	{
		try
		{
			...
			if (count == 1 && TypeChecker.CheckTypes<FairyGUI.ObjectType>(L, 1))
			{
			...
			else if (count == 1 && TypeChecker.CheckTypes<FairyGUI.PackageItem>(L, 1))
			{

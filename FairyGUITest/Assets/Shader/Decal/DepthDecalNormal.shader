...
Shader "Decal/DepthDecalNormal"
{
	Properties
	{
		_MainTex("Texture", 2D) = "white" {}
		_NormalClipThreshold( "_NormalClipThreshold" , Float ) = 1
	}
	...
	SubShader
	{
		...
		v2f vert(a2v v)
		{
			...
			o.zDir = UnityObjectToViewPos(float3(0,0,1));
		...
		fixed4 frag(v2f i) : SV_Target
		{
			...
			//normals为视空间的normals
			float3 normals = DecodeViewNormalStereo(tex2D(_CameraDepthNormalsTexture , screenUv));
			normals = normals * 2 - 1;
			...
			//如果与z平行，抛弃该点
			float3 zDir = normalize(i.zDir);
			clip( dot(zDir, normals) - _NormalClipThreshold);
